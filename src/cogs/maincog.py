...
class MainCog(commands.Cog):
    ...
    async def list_specialties(self, ctx):
        ...
        for index, specialty in enumerate(self.specialties):
            embed.add_field(name=index, value=f"** {specialty} **", inline=False)
    ...
    async def get_specialty(self, ctx, params):
        ...
        message += (f"Roles `{', '.join(added_specialties)}` were added to {author.name}") if len(added_specialties) > 0 else "No ranks were added"
    ...
    async def remove_specialty(self, ctx, params):
        ...
        message += (f"Roles `{', '.join(added_specialties)}` were removed from {author.name}") if len(added_specialties) > 0 else "No ranks were removed"
